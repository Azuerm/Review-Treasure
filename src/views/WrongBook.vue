...
const title = computed(() => {
  if (props.allMode) return '全部错题'
  return (library.value?.name || '') + ' - 错题本'
})
...
function loadCards() {
  let cards
  if (props.allMode) {
    cards = store.getAllWrongCards()
  } else {
    cards = store.getWrongCards(libraryId.value)
  }
...
function rateScore(score) {
  ...
  if (score >= 2) {
    showNotify({ type: 'success', message: '已从错题本移除' })
  }
  nextCard()
}
...
<template>
  <div class="review-page">
    <van-nav-bar :title="title" left-arrow @click-left="router.back()">
      <template #right>
        <van-tag type="danger" size="medium">{{ allCards.length }} 题</van-tag>
      </template>
    </van-nav-bar>

    <div class="progress-bar">
      <van-progress :percentage="progress" stroke-width="4" :show-pivot="false" color="#ee0a24" />
      <span class="progress-text">{{ currentIndex + 1 }} / {{ allCards.length }}</span>
    </div>

    <div v-if="finished" class="finish-screen">
      <van-empty image="https://fastly.jsdelivr.net/npm/@vant/assets/custom-empty-image.png" description="错题本已清空！">
    ...
    <div v-else class="empty-state">
      <van-empty description="没有错题，继续保持！" />
    </div>
